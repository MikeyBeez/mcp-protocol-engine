...
export class ProtocolEngine {
    ...
    matchesTrigger(input, context, trigger) {
        switch (trigger.type) {
            case 'phrase':
    ...
    startProtocol(protocolId, context = {}) {
    ...
    getNextAction(activeProtocolId) {
    ...
    completeStep(activeProtocolId, stepId, result) {
        const active = this.activeProtocols.get(activeProtocolId);
        if (!active) {
            throw new Error(`Active protocol ${activeProtocolId} not found`);
        }
    ...
    displayProgress(activeProtocolId) {
        const active = this.activeProtocols.get(activeProtocolId);
        if (!active) {
    ...
    listProtocols(category) {
        ...
        return filtered.map(p => ({
            ...
            triggers: p.triggers.map(t => typeof t.pattern === 'string' ? t.pattern : 'custom pattern'),
